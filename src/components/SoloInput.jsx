...
function MascotTip() {
  const tips = useMemo(() => FOOD_TIP_TEXTS, []);
...
export default function SoloInput({ onSearch, onFortune, isLoading, initialCategory }) {
  ...
  const getModesByCategory = (categoryKey) => Object.entries(soloModes).filter(([_key, mode]) => mode.category === categoryKey);
